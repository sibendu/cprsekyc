										...
										var c = connection
												.query(
														...
														function(err, rows) {
															...
															var url = "http://141.144.29.26:9073/crm/customer?action=updatestatus&email="
																	+ rows[0].EMAIL
																	+ "&status=Verified";
...
exports.check = function(req, res, next) {
	...
	req.getConnection(function(err, connection) {
		...
		var query = connection.query('SELECT * FROM customer WHERE name = ?',
				[ name ], function(err, rows) {
					...
					if (rows[0] == null) {
						...
						var qry = connection.query(
								"INSERT INTO customer set ? ", data, function(
										err, newrows) {
									if (err)
										console.log("Error inserting : %s ",
												err);
